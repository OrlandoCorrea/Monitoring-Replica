...
ID=$1
ALL=$(mysql --connect-timeout=1 -D$MYDB -u$MYUSER -p$MYPASS -se "SELECT concat(dbtype,'|',customer,'|',description,'|',sid,'|',ip_ci,'|',ip_replica,'|',user_ci,'|',range_log) FROM replicas_init where ID='$ID';")
...
EOF
                ...
                if [ $status2 -ne 0 ];then
                        ...
                        ssh -q -o "StrictHostKeyChecking no" $user_unix@$ip_ci "rm -f result_primary.txt " 2>&1 >/dev/null
                        current_primary_seq=$(echo "$execute_shell_pri" | awk ' FNR == 15 { print $2 } ')
                        name_pri=$(echo "$execute_shell_pri" | awk ' FNR == 15 { print $1 } ')
                        applied_pri=$(echo "$execute_shell_pri" | awk ' FNR == 15 { print $3 } '| tr -d $'\r')
                        shipped_pri=$(echo "$execute_shell_pri" | awk ' FNR == 15 { print $4 } '| tr -d $'\r')
...
EOF
                                        ...
                                        if [ $status5 -ne 0 ];then
                                                ...
                                                ssh -A -T $user_unix@$ip_ci ssh -A -T $user_unix@$ip_replica "rm -f result_replica.txt" 2>&1 >/dev/null
                                                applied_repl=$(echo "$execute_shell_repl" | awk ' FNR == 14 { print $6 } '| tr -d $'\r')
                                                received_repl=$(echo "$execute_shell_repl" | awk ' FNR == 15 { print $6 } ' | tr -d $'\r')
                                                time_received=$(echo "$execute_shell_repl" | awk ' FNR == 15 { print $4 } ')
                                                time_applied=$(echo "$execute_shell_repl" | awk ' FNR == 14 { print $4 } ')
